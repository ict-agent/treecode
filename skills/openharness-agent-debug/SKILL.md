---
name: openharness-agent-debug
description: How to use the OpenHarness `agent-debug` CLI to programmatically test, trace, and debug OpenHarness agent sessions from an external LLM or automated script. Covers session lifecycle, all output file formats, and common usage patterns.
---

# OpenHarness Agent Debug Skill

Use `oh agent-debug` when you need to:
- Write automated E2E tests against OpenHarness
- Trace what OpenHarness's agent actually does in response to a prompt
...
uv run oh agent-debug start my-test --verbose
```

Files are created under `.openharness/sessions/my-test/`.

### 2. Send messages

```bash
# Plain text prompt (auto-wrapped into the submit_line protocol)
uv run oh agent-debug send my-test "list the files in the current directory"
...
uv run oh agent-debug send my-test '{"type": "submit_line", "line": "hello"}'
```

`send` is **synchronous** — it blocks until the agent emits `line_complete`, then prints all `OHJSON:` events and exits. This makes it trivially script-able.
...
All files live in `.openharness/sessions/<ID>/`.

### `output`
Raw NDJSON stream from the backend. Every line is prefixed `OHJSON:` and contains one event object. Useful for machine parsing and assertions.
...
**Example entry:**
```json
OHJSON:{"type":"assistant_complete","message":"The file contains...","item":{"role":"assistant","text":"..."}}
```
...
```bash
echo '{"type":"submit_line","line":"hello"}' > .openharness/sessions/my-test/input
```
...
def oh_send(session_id: str, message: str) -> str:
    result = subprocess.run(
        ["uv", "run", "oh", "agent-debug", "send", session_id, message],
        capture_output=True, text=True
    )
    return result.stdout

# Setup
subprocess.run(["uv", "run", "oh", "agent-debug", "start", "e2e-test"])
oh_send("e2e-test", "/permissions set full_auto")

# Exercise
output = oh_send("e2e-test", "what is 2 + 2?")
...
# Teardown
subprocess.run(["uv", "run", "oh", "agent-debug", "stop", "e2e-test"])
```
