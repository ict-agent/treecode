...
This page collects concrete ways to use TreeCode without overselling the project. Each example is intended to be small, reproducible, and easy to extend.

## 1. Repository-aware coding assistant

Use TreeCode as a lightweight local coding agent for reading code, making edits, and running validation commands.
...
```bash
uv run treecode -p "Summarize the purpose of this repository" --output-format json
uv run treecode -p "List files that define the permission system" --output-format stream-json
```

## 3. Skill and plugin playground

TreeCode can load Markdown skills and Claude-style plugin layouts, which makes it useful for experimentation with custom workflows.

Examples:

- Put a custom skill in `~/.treecode/skills/`.
- Install a plugin into `~/.treecode/plugins/`.
...
TreeCode is useful when you need to compare Anthropic-compatible backends behind one harness.
...
- [`CLAUDE.md`](../CLAUDE.md) (if present in your clone) for contributor navigation.
...
- Focused on what TreeCode makes easier, not on generic LLM claims.
