...
Agent Loop 是 TreeCode 的心脏——它实现了「用户输入 → LLM 思考 → 工具调用 → 结果回传 → 继续思考」的核心循环。

> 对应源码：`src/treecode/engine/`
...
> 源码：[`engine/query.py`](../src/treecode/engine/query.py)
...
> 源码：[`engine/query_engine.py`](../src/treecode/engine/query_engine.py)
...
> 源码：[`engine/messages.py`](../src/treecode/engine/messages.py)
...
> 源码：[`engine/stream_events.py`](../src/treecode/engine/stream_events.py)
