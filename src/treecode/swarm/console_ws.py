...
from treecode.ui.repl_input_history import append_repl_input_history_line, load_repl_input_history_lines
...
class SwarmConsoleWsServer:
    ...
    async def _dispatch(self, websocket: ServerConnection, message: ConsoleClientMessage) -> None:
        ...
        try:
            tc = await self._maybe_handle_tc_command(message.command, message.payload)
            if tc is not None:
                message_type, handler_result, should_broadcast = tc
            else:
    ...
    async def _maybe_handle_tc_command(
        self, command: str, payload: dict[str, Any]
    ) -> tuple[str, dict[str, Any], bool] | None:
        """Handle TreeCode SessionHost commands when integrated with ``treecode``."""
        if self._session_host is None:
            return None
        if command == "tc_submit_line":
            line = str(payload.get("line", "")).strip()
            if not line:
                raise ValueError("tc_submit_line requires non-empty line")
        ...
        if command == "tc_permission_response":
            ...
            return "ack", {"ok": True}, False
        if command == "tc_question_response":
            ...
            return "ack", {"ok": True}, False
        if command == "tc_set_selected_agent":
