"""Persistent team lifecycle management for TreeCode swarms.
...
from treecode.swarm.mailbox import get_team_dir
from treecode.swarm.types import BackendType
...
async def _kill_orphaned_teammate_panes(team_name: str) -> None:
    ...
    from treecode.swarm.spawn_utils import is_inside_tmux
    from treecode.swarm.types import is_pane_backend
...
class TeamLifecycleManager:
    ...
    Persists team metadata to ``~/.treecode/teams/<name>/team.json``.
    ...
    def list_teams(self) -> list[TeamFile]:
        """Return all teams found in ``~/.treecode/teams/``, sorted by name."""
        base = Path.home() / ".treecode" / "teams"
        if not base.exists():
            return []
