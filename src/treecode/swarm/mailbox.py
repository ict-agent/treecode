"""File-based async message queue for leader-worker communication in TreeCode swarms.

Each message is stored as an individual JSON file:
    ~/.treecode/teams/<team>/agents/<agent_id>/inbox/<timestamp>_<message_id>.json
...
def get_team_dir(team_name: str) -> Path:
    """Return ~/.treecode/teams/<team_name>/"""
    base = Path.home() / ".treecode" / "teams" / team_name
    base.mkdir(parents=True, exist_ok=True)
    return base


def get_agent_mailbox_dir(team_name: str, agent_id: str) -> Path:
    """Return ~/.treecode/teams/<team_name>/agents/<agent_id>/inbox/"""
