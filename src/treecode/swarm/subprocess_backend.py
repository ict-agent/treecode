...
from treecode.swarm.spawn_utils import (
...
from treecode.tasks.manager import get_task_manager, load_persisted_task_record
...
class SubprocessBackend:
    ...
    - "oneshot" (default): runs ``treecode -p <prompt> --output-format stream-json``.
      One-shot, exits after completing the prompt. Use for most tasks.
    - "persistent": runs ``treecode --backend-only``.
    ...
    def _build_swarm_metadata(self, config: TeammateSpawnConfig) -> dict[str, str]:
        ...
        leader = config.leader_session_id or os.environ.get("TREECODE_SWARM_LEADER_SESSION_ID") or ""
        meta = {
            "TREECODE_SWARM_AGENT_ID": f"{config.name}@{config.team}",
            "TREECODE_SWARM_PARENT_AGENT_ID": config.parent_agent_id or "",
            "TREECODE_SWARM_ROOT_AGENT_ID": config.resolved_root_agent_id(),
            "TREECODE_SWARM_PARENT_SESSION_ID": config.parent_session_id,
            "TREECODE_SWARM_SESSION_ID": config.session_id or f"{config.name}@{config.team}",
            "TREECODE_SWARM_LINEAGE_PATH": "::".join(config.resolved_lineage_path()),
        }
        if leader:
            meta["TREECODE_SWARM_LEADER_SESSION_ID"] = leader
        return meta
    ...
    async def _notify_leader_on_completion(
        ...
        from treecode.swarm.mailbox import TeammateMailbox, create_idle_notification
