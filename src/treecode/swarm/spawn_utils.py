...
TEAMMATE_COMMAND_ENV_VAR = "TREECODE_TEAMMATE_COMMAND"
...
def get_teammate_command() -> str:
    ...
    Resolution order:
    1. ``TREECODE_TEAMMATE_COMMAND`` environment variable — allows the
       operator to point at a specific binary or wrapper script.
    2. The ``treecode`` entry-point on PATH (installed package mode).
    3. The current Python interpreter running the ``treecode`` module
    ...
    entry_point = shutil.which("treecode")
    if entry_point:
        return entry_point
...
def build_inherited_env_vars(swarm_metadata: dict[str, str] | None = None) -> dict[str, str]:
    ...
    Always includes ``TREECODE_AGENT_TEAMS=1`` plus any provider/proxy
