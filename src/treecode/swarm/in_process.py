...
* :class:`InProcessBackend` – implements
  :class:`~treecode.swarm.types.TeammateExecutor` and manages the dict of
  live asyncio Tasks.
"""
...
from treecode.swarm.context_registry import AgentContextSnapshot, get_context_registry
...
async def start_in_process_teammate(
    *,
    ...
    2. Drives the query engine loop (reusing
       :func:`~treecode.engine.query.run_query`).
    ...
    query_context:
        Optional pre-built
        :class:`~treecode.engine.query.QueryContext`.  When *None* this
        function tries to build a full runtime (unless
        ``TREECODE_TEAMMATE_USE_STUB=1``), then falls back to a stub.
    message_queue:
    ...
    try:
        ...
        own_bundle = None
        if resolved_qc is None and os.environ.get("TREECODE_TEAMMATE_USE_STUB") != "1":
            try:
                from treecode.swarm.teammate_runtime import build_teammate_runtime_bundle
...
async def _run_query_loop(
    ...
    """Drive :func:`~treecode.engine.query.run_query` until done or cancelled.
    ...
    from treecode.engine.query import run_query
    from treecode.engine.stream_events import ToolExecutionStarted
...
class InProcessBackend:
    ...
    async def send_message(self, agent_id: str, message: TeammateMessage) -> None:
        ...
        from treecode.swarm.mailbox import MailboxMessage
