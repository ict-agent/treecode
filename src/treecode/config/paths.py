"""Path resolution for TreeCode configuration and data directories.

Follows XDG-like conventions with ~/.treecode/ as the default base directory.
"""
...
def get_config_dir() -> Path:
    ...
    Resolution order:
    1. TREECODE_CONFIG_DIR environment variable
    2. ~/.treecode/
    """
    env_dir = os.environ.get("TREECODE_CONFIG_DIR")
    if env_dir:
...
def get_config_file_path() -> Path:
    """Return the path to the main settings file (~/.treecode/settings.json)."""
...
def get_data_dir() -> Path:
    ...
    Resolution order:
    1. TREECODE_DATA_DIR environment variable
    2. ~/.treecode/data/
    """
    env_dir = os.environ.get("TREECODE_DATA_DIR")
    if env_dir:
...
def get_logs_dir() -> Path:
    ...
    Resolution order:
    1. TREECODE_LOGS_DIR environment variable
    2. ~/.treecode/logs/
    """
    env_dir = os.environ.get("TREECODE_LOGS_DIR")
    if env_dir:
...
def get_project_config_dir(cwd: str | Path) -> Path:
    """Return the per-project .treecode directory."""
    project_dir = Path(cwd).resolve() / ".treecode"
