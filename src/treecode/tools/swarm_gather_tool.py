...
from treecode.swarm.gather_spec import GatherSpec, load_gather_spec
...
class SwarmGatherToolInput(BaseModel):
    ...
    spec_name: str | None = Field(
        default=None,
        description="Optional gather spec name from .treecode/gather/*.md. Defaults to a generic inline gather spec.",
    )
...
def build_engine_gather_local_runner(engine: QueryEngine) -> LocalGatherRunner:
    ...
    async def _runner(
        *,
        ...
        from treecode.engine.query_engine import QueryEngine as QueryEngineCls
