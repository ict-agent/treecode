...
if TYPE_CHECKING:
    from treecode.commands.registry import CommandContext, CommandResult


async def set_max_turns_handler(args: str, ctx: CommandContext) -> CommandResult:
    """Handle /set-max-turns command."""
    from treecode.commands.registry import CommandResult
    ...
    # 持久化到 settings.json
    from treecode.config.settings import load_settings, save_settings
