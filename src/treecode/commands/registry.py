...
from treecode.bridge import get_bridge_manager
from treecode.bridge.types import WorkSecret
...
from treecode.coordinator.agent_definitions import get_agent_definition, get_all_agent_definitions
...
from treecode.output_styles import load_output_styles
from treecode.permissions import PermissionChecker, PermissionMode
from treecode.plugins import load_plugins
from treecode.prompts import build_runtime_system_prompt
from treecode.swarm.debugger import LIVE_MAIN_AGENT_ID
from treecode.plugins.installer import install_plugin_from_path, uninstall_plugin
from treecode.services import (
    compact_messages,
...
from treecode.services.session_storage import get_project_session_dir, load_session_snapshot
from treecode.skills import load_skill_registry
from treecode.tasks import get_task_manager
from treecode.commands.execute_script import load_execute_lines
from treecode.tools.agent_tool import AgentTool, AgentToolInput
from treecode.tools.base import ToolExecutionContext
from treecode.tools.swarm_gather_tool import (
    SwarmGatherTool,
    SwarmGatherToolInput,
    build_contextual_gather_local_runner,
    build_engine_gather_local_runner,
)
from treecode.commands.set_max_turns_command import set_max_turns_handler

if TYPE_CHECKING:
    from treecode.state import AppStateStore
...
class CommandRegistry:
    ...
    def format_help_list(self) -> str:
        ...
        from treecode.commands.slash_catalog import format_help_list as _format_help_list
    ...
    def help_topic(self, topic: str) -> str | None:
        ...
        from treecode.commands.slash_catalog import format_help_topic
...
def _resolve_spawn_parent_snapshot(
    ...
    *,
    ...
    if leader_session_id and live_nodes:
        ...
        from treecode.swarm.session_scope import filter_live_nodes_for_leader_session
...
def create_default_command_registry() -> CommandRegistry:
    ...
    async def _version_handler(_: str, context: CommandContext) -> CommandResult:
        del context
        try:
            version = importlib.metadata.version("treecode")
        except importlib.metadata.PackageNotFoundError:
            version = "0.1.0"
        return CommandResult(message=f"TreeCode {version}")
    ...
    async def _resume_handler(args: str, context: CommandContext) -> CommandResult:
        from treecode.services.session_storage import list_session_snapshots, load_session_by_id
    ...
    async def _agents_handler(args: str, context: CommandContext) -> CommandResult:
        from treecode.session_host_registry import get_active_session_host
        ...
        if session_host is None or getattr(session_host, "debugger", None) is None:
            return CommandResult(
                message=(
                    "Persistent agents are only available in the shared TreeCode session.\n"
    ...
    async def _agent_defs_handler(args: str, context: CommandContext) -> CommandResult:
        ...
        if tokens[0] == "path":
            return CommandResult(
                message=(
                    ...
                    "3. builtin  src/treecode/coordinator/agent_definitions.py"
                )
            )
        ...
        if tokens[0] == "init":
            ...
            target.write_text(
                ...
                "---\n\n"
                "You are a reusable TreeCode agent profile.\n\n"
    ...
    async def _spawn_handler(args: str, context: CommandContext) -> CommandResult:
        ...
        if session_host is None or getattr(session_host, "debugger", None) is None:
            return CommandResult(
                message="`/spawn` requires the shared TreeCode session. Start `treecode` / the shared web console first."
            )
    ...
    async def _init_handler(args: str, context: CommandContext) -> CommandResult:
        ...
        if not claudemd.exists():
            claudemd.write_text(
                ...
                "- Use TreeCode tools deliberately.\n"
        ...
        for relative, content in (
            (
                ...
                "# Project TreeCode Config\n\nThis directory stores project-specific TreeCode state.\n",
            ),
            (
        ...
        if not created:
            return CommandResult(message="Project already initialized for TreeCode.")
    ...
    async def _login_handler(args: str, context: CommandContext) -> CommandResult:
        ...
        save_settings(settings)
        return CommandResult(message="Stored API key in ~/.treecode/settings.json")
    ...
    async def _keybindings_handler(_: str, context: CommandContext) -> CommandResult:
        from treecode.keybindings import get_keybindings_path, load_keybindings
    ...
    async def _voice_handler(args: str, context: CommandContext) -> CommandResult:
        from treecode.voice import extract_keyterms, inspect_voice_capabilities
    ...
    async def _privacy_settings_handler(_: str, context: CommandContext) -> CommandResult:
        ...
        lines = [
            ...
            "- storage: local files under ~/.treecode and project .treecode",
        ]
        return CommandResult(message="\n".join(lines))
    ...
    async def _release_notes_handler(_: str, context: CommandContext) -> CommandResult:
        ...
        return CommandResult(
            message=(
                "# Release Notes\n\n"
                "- React TUI is now the default `treecode` interface.\n"
    ...
    registry.register(SlashCommand("exit", "Exit TreeCode", _exit_handler))
    registry.register(SlashCommand("clear", "Clear conversation history", _clear_handler))
    registry.register(SlashCommand("version", "Show the installed TreeCode version", _version_handler))
    ...
    registry.register(SlashCommand("init", "Initialize project TreeCode files", _init_handler))
    ...
    registry.register(SlashCommand("release-notes", "Show recent TreeCode release notes", _release_notes_handler))
