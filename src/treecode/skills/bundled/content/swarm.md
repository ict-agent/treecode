...
- Do **not** rebuild the current tree by scanning `~/.treecode/data/swarm/contexts/`; that directory is historical cache and may include stale, finished, or unrelated agents.
...
**Naming:** Use `subagent_type` for the capability profile and `agent_name` for the visible runtime identity. If the user explicitly says “create A / A1 / A2”, set `agent_name` to that exact requested name. If the id `name@team` is already taken, TreeCode auto-renames to `name-1@team`, `name-2@team`, etc.
