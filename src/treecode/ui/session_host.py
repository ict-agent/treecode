...
from treecode.swarm.debugger import (
    LIVE_MAIN_AGENT_ID,
    SwarmDebuggerService,
    create_default_swarm_debugger_service,
)
from treecode.tasks.agent_tasks import count_agent_tasks_for_cwd, list_agent_tasks_for_cwd
from treecode.tasks.types import TaskRecord
from treecode.commands.slash_catalog import catalog_dicts
from treecode.ui.protocol import BackendEvent, FrontendRequest, SlashCatalogEntry, TaskSnapshot, TranscriptItem, _state_payload
from treecode.ui.runtime import RuntimeBundle, build_runtime, close_runtime, handle_line, start_runtime
...
class SessionHost:
    ...
    async def start(self) -> None:
        ...
        if not os.environ.get("TREECODE_SWARM_AGENT_ID"):
            swarm_tool_metadata = {
    ...
    def build_session_resync_event(self) -> BackendEvent:
        """Full session snapshot for Web UI (same payload as session_resync)."""
        assert self._bundle is not None
    ...
    async def _handle_list_sessions(self) -> None:
        from treecode.services.session_storage import list_session_snapshots
