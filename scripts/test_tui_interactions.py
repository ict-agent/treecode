...
def test_command_picker_shows() -> tuple[bool, str]:
    ...
    try:
        ...
        has_welcome = " _____ ____  _____ _____" in output
        if has_welcome:
...
def test_permission_flow() -> tuple[bool, str]:
    ...
    env["TREECODE_FRONTEND_SCRIPT"] = json.dumps([
        "Create a file called /tmp/tc_permission_test.txt with content 'test'",
    ])
...
def test_no_headless_flag() -> tuple[bool, str]:
    ...
    result = subprocess.run(
        [sys.executable, "-m", "treecode", "--help"],
