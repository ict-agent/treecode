...
PLUGINS_REPO = Path("/tmp/treecode-test-plugins/plugins")
...
def _run_treecode(*args: str, timeout: int = 90, cwd: str | None = None) -> subprocess.CompletedProcess:
    cmd = [sys.executable, "-m", "treecode", *args]
...
async def test_install_real_skills() -> tuple[bool, str]:
    """Copy real skills from anthropics/skills into treecode user skills dir."""
...
async def test_skills_in_prompt_with_real() -> tuple[bool, str]:
    ...
    from treecode.prompts.context import build_runtime_system_prompt
...
async def test_install_real_plugins() -> tuple[bool, str]:
    """Copy real plugins into treecode plugin directory."""
    if not PLUGINS_REPO.exists():
        return False, f"Plugins repo not found at {PLUGINS_REPO}"

    dest_base = PROJECT_ROOT / ".treecode" / "plugins"
...
async def test_real_plugins_loaded() -> tuple[bool, str]:
    ...
    from treecode.config.settings import load_settings
    from treecode.plugins.loader import load_plugins
...
async def test_plugin_hook_structure() -> tuple[bool, str]:
    ...
    dest = PROJECT_ROOT / ".treecode" / "plugins" / "security-guidance"
...
async def test_commit_command_content() -> tuple[bool, str]:
    ...
    dest = PROJECT_ROOT / ".treecode" / "plugins" / "commit-commands"
